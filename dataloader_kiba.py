...
# 读取文件(训练集和测试集)
kiba_train_drugs,kiba_train_proteins,kiba_train_Y = read_data("kiba_train.csv")
kiba_test_drugs,kiba_test_proteins,kiba_test_Y = read_data("kiba_test.csv")
drugs = kiba_train_drugs+kiba_test_drugs
print(len(drugs)) # 118254
...
train_prots = [seq_cat(t) for t in kiba_train_proteins]
test_prots = [seq_cat(t) for t in kiba_test_proteins]
print(len(train_prots))  # 98545
print(len(test_prots))   # 19709
...
for smile in drugs:
    ...
    compounds.append(lg)
print("原始SMILES数量：", len(compounds))  # 118254
smile_graph = {}
compound_smiles = set(compounds)
print("不重复的SMILES数量:", len(compound_smiles))   # 2068
for smile in compound_smiles:
    g = smile_to_graph(smile) # c_size, features, edge_index
    smile_graph[smile] = g
print("simile_graph长度:", len(smile_graph))  # 2068
...
print(len(cluster))  # 2068
print(len(edges))   # 2068
...
print("clique_graph长度:", len(clique_graph))  # 2068

train_drugs, train_prots,  train_Y = np.asarray(kiba_train_drugs), np.asarray(train_prots), np.asarray(kiba_train_Y)
test_drugs, test_prots,  test_Y = np.asarray(kiba_test_drugs), np.asarray(test_prots), np.asarray(kiba_test_Y)

train_data = TestbedDataset(root='data', dataset='kiba_train', xd=train_drugs, xt=train_prots, y=train_Y,smile_graph=smile_graph,clique_graph=clique_graph)
test_data = TestbedDataset(root='data', dataset='kiba_test', xd=test_drugs, xt=test_prots, y=test_Y,smile_graph=smile_graph,clique_graph=clique_graph)
